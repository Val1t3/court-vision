...
def _coerce_points(obj) -> np.ndarray:
    ...
    for item in obj:
        if isinstance(item, dict) and "x" in item and "y" in item:
            ...
            raise ValueError("Unsupported point format in JSON; expected [x,y] or {x,y}.")
...
def load_points_json(path: str) -> np.ndarray:
    ...
    if pts.shape[0] < 4:
        raise ValueError("Need at least 4 correspondence points to compute a homography.")
    return pts
...
def compute_homography(image_points_xy: np.ndarray, court_points_xy_m: np.ndarray) -> np.ndarray:
    ...
    H, mask = cv2.findHomography(image_points_xy.astype(np.float32), court_points_xy_m.astype(np.float32), method=cv2.RANSAC, ransacReprojThreshold=3.0)
    if H is None:
...
# -----------------------------
# Distances on projected (meter) tracks
# -----------------------------
...
def distance_euclidean_meters(df_proj: pd.DataFrame) -> Dict[int, float]:
    ...
    for pid, g in df_proj.sort_values(["id", "frame"]).groupby("id"):
        ...
        totals[pid] = float(np.nansum(deltas))
    return totals
...
def distance_kalman_meters(df_proj: pd.DataFrame, cfg: KFConfig) -> Dict[int, float]:
    ...
    for pid, g in df_proj.sort_values(["id", "frame"]).groupby("id"):
        ...
        if len(arr) < 2:
            totals[pid] = 0.0
            continue
        confs = g["confidence"].to_numpy(dtype=float) if "confidence" in g.columns else np.ones(len(arr))
        ...
        deltas = np.sqrt(np.sum(np.diff(sm, axis=0)**2, axis=1))
        totals[pid] = float(np.nansum(deltas))
...
# -----------------------------
# Optical Flow accumulation in meters
# -----------------------------
...
def distance_optical_flow_meters(
    ...
    valid: Dict[int, bool] = {}
    totals_m: Dict[int, float] = {pid: 0.0 for pid in df_det["id"].unique()}
        ...
        if pids:
            ...
            for j, pid in enumerate(pids):
                if st[j] == 1:
                    ...
                    move_m = float(np.hypot(*(new_xy_m - old_xy_m)))
                    totals_m[pid] += move_m
                    ...
                    if frame_idx in frames_group:
                        ...
                        det = g[g["id"] == pid]
                        if len(det) > 0:
                            cx, by = float(det.iloc[0]["feet_x"]), float(det.iloc[0]["feet_y"])
                            ...
                            pts_img[pid] = np.array([[[new_px[0], new_px[1]]]], dtype=np.float32)
                    else:
                        pts_img[pid] = np.array([[[new_px[0], new_px[1]]]], dtype=np.float32)
                    valid[pid] = True
                else:
        ...
        if frame_idx in frames_group:
            ...
            for _, row in g.iterrows():
                ...
                if pid not in valid or not valid[pid]:
                    pts_img[pid] = np.array([[[row["feet_x"], row["feet_y"]]]], dtype=np.float32)
                    valid[pid] = True
...
def convert_schema_px_to_meters(court_pts_schema: np.ndarray, length_m: float, width_m: float) -> np.ndarray:
...
def prepare_projected_dataframe(
    ...
    df = detections.copy()
    feet_xy = df.apply(lambda r: bbox_feet_point(r, feet_mode), axis=1, result_type="expand")
...
def run_all(
    ...
    if court_units == "schema_px":
        court_pts_m = convert_schema_px_to_meters(court_pts_in, schema_length_m, schema_width_m)
    ...
    eucl = distance_euclidean_meters(df_proj)
    kf = distance_kalman_meters(df_proj, KFConfig(fps=fps))
    ...
    if video_path:
        of = distance_optical_flow_meters(video_path, df_proj, H)
...
def main():
    parser = argparse.ArgumentParser(description="Player distance in meters using homography (Euclidean, Kalman, Optical Flow)")
    parser.add_argument("--csv", required=True, help="Path to detections CSV")
    parser.add_argument("--img-points", required=True, help="JSON with image points (pixels)")
    parser.add_argument("--court-points", required=True, help="JSON with corresponding court points (meters or schema pixels)")
    parser.add_argument("--video", default=None, help="Path to video (required for optical flow)")
    parser.add_argument("--fps", type=float, default=25.0, help="FPS (used if --video omitted)")
    parser.add_argument("--feet-point", choices=["bottom_center", "bottom_mid", "feet", "center"], default="bottom_center")
    parser.add_argument("--court-units", choices=["meters", "schema_px"], default="schema_px",
                        help="Units used in court points JSON. If schema_px, provide physical dimensions.")
    parser.add_argument("--schema-length-m", type=float, default=28.0, help="Physical court length in meters (x axis) when court-units=schema_px")
    parser.add_argument("--schema-width-m", type=float, default=15.0, help="Physical court width in meters (y axis) when court-units=schema_px")
